...
`test/select_baseline_points.py` 依据原 `RM_GridMap` 的 0.35 m 圆形膨胀、`occtopo` 有效区、最大 8 邻接自由连通域及两次 BFS 确定选点；ROS1/ROS2 都记录并断言相同选择。当前点对为 `(-0.919, -4.454) → (-2.719, 1.946)`，种子为 `7`；产物保存为 JSON，比较末端误差、采样路径长度和总时间。

| 指标 | ROS1 Noetic | ROS2 Humble | 结果 |
| --- | ---: | ---: | --- |
| 多项式段数 | 19 | 19 | 一致 |
| 终点 `(x, y)` | `(-2.714626, 1.949258)` | `(-2.714626, 1.949258)` | 误差 0 m |
| 采样路径长度 | 10.844724 m | 10.844724 m | 相对误差 0% |
| 总时长 | 6.723219 s | 6.723219 s | 相对误差 0% |
