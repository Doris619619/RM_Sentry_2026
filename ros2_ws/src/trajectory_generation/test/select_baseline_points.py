#!/usr/bin/env python3
"""Deterministically choose a Planning baseline pair from the real map.

The candidate set is the largest 8-connected free component after applying the
same 0.35 m circular inflation as ``RM_GridMap::swellOccMap``.  It is further
restricted to the original ``occtopo`` support, so map padding cannot become a
test start or goal.  Two BFS sweeps select a stable, well-separated pair.
"""
...
def select_points(map_directory):
    ...
    return {
        'algorithm': 'largest_8_connected_inflated_occtopo_two_bfs',
        'component_size': len(component),
        'inflation_cells': inflation,
        'start': _coord(first),
        'goal': _coord(second),
