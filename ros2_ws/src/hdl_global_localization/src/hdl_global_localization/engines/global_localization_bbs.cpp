...
GlobalLocalizationBBS::GlobalLocalizationBBS(ros::NodeHandle& private_nh) : private_nh(private_nh) {
  gridmap_pub = private_nh.advertise<nav_msgs::msg::OccupancyGrid>("bbs/gridmap", 1, true);
  map_slice_pub = private_nh.advertise<sensor_msgs::msg::PointCloud2>("bbs/map_slice", 1, true);
  scan_slice_pub = private_nh.advertise<sensor_msgs::msg::PointCloud2>("bbs/scan_slice", 1, false);
}
...
void GlobalLocalizationBBS::set_global_map(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud) {
  ...
  pcl::toROSMsg(*map_3d, map_message);
  map_message.header.frame_id = "map";
  map_slice_pub.publish(map_message);
...
GlobalLocalizationResults GlobalLocalizationBBS::query(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, int max_num_candidates) {
  ...
  if (scan_slice_pub.getNumSubscribers()) {
    ...
    sensor_msgs::msg::PointCloud2 scan_message;
    pcl::toROSMsg(*scan_3d, scan_message);
    scan_message.header.frame_id = "base_link";
    scan_slice_pub.publish(scan_message);
  }
