...
// 此函数用于接收左 MID360 的 Livox CustomMsg；输入为逐点时间点云，输出为 Point-LIO 的 LiDAR 缓存并唤醒处理循环。
...
// 此函数用于按 LiDAR 时间线缓存左 MID360 IMU；输入为原始 IMU，输出为受缓存时长限制的 IMU 队列并唤醒处理循环。
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
    ...
    while (!imu_deque.empty() && timestamp - get_time_sec(imu_deque.front()->header.stamp) > imu_buffer_seconds) {
        imu_deque.pop_front();
    }
...
// 此函数用于初始化 Point-LIO 节点、ROS 通信和处理循环；输入为 ROS 参数，输出为里程计、点云与 TF。
int main(int argc, char **argv) {
    ...
    // 此处用于按雷达类型创建低延迟订阅；Livox 默认使用 CustomMsg 以保留逐点时间，IMU 缓存深度由参数计算。
    ...
    const auto lidar_qos = rclcpp::QoS(rclcpp::KeepLast(lidar_qos_depth)).best_effort().durability_volatile();
    const auto imu_qos = rclcpp::QoS(rclcpp::KeepLast(imu_qos_depth)).best_effort().durability_volatile();
    if (p_pre->lidar_type == AVIA) {
        sub_pcl_livox = nh->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, lidar_qos, livox_pcl_cbk);
    } else {
