...
def generate_test_description():
    return launch.LaunchDescription([production_launch(False), launch_testing.actions.ReadyToTest()])


class TestProductionSafeMode(GraphTest):
    def test_safe_mode_excludes_physical_sensor_pipeline_and_global_server(self):
        self.assertTrue(self.wait_for(lambda: {'hdl_localization', 'globalmap_server', 'trajectory_tracking', 'hit_bridge', 'strategy_node'} <= self.node_names()))
        ...
        self.assertNotIn('hdl_global_localization', names)
        self.assertNotIn('/relocalize', [name for name, _ in self.node.get_service_names_and_types()])
        self.assertFalse(self.hdl_global_localization_enabled())
