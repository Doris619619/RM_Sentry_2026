...
void TrackingGridMap::initGridMap(ros::NodeHandle &nh, std::string occ_map_file, std::string bev_map_file, std::string distance_map_file,
                            ...
                            double _2d_search_height_low, double _2d_search_height_high, double _search_radius)
{
    ...
    m_local_point_sub = m_node.subscribe<sensor_msgs::PointCloud2>("/cloud_registered_world", 1, &TrackingGridMap::lidarCloudCallback, this);
    m_odometry_sub = m_node.subscribe<nav_msgs::Odometry>("/odometry_imu", 1, &TrackingGridMap::odometryCallback, this);
    m_realsense_sub =  m_node.subscribe<sensor_msgs::PointCloud2>("/realsense_pointcloud", 1, &TrackingGridMap::realsenseCloudCallback, this);

    gazebo_point_sub = m_node.subscribe<sensor_msgs::PointCloud2>("/mbot/velodyne_points", 1, &TrackingGridMap::gazeboCloudCallback, this);
    gazebo_odometry_sub = m_node.subscribe<gazebo_msgs::ModelStates>("/gazebo/model_states", 1, &TrackingGridMap::gazeboPosCallback, this);

    global_map_pub = m_node.advertise<sensor_msgs::PointCloud2>("grid_map_vis", 1);

//    m_occ_timer = m_node.createTimer(ros::Duration(0.051), &TrackingGridMap::updateLocalMap, this);
...
void TrackingGridMap::gazeboPosCallback(const gazebo_msgs::ModelStatesConstPtr &state)
{
...
void TrackingGridMap::odometryCallback(const nav_msgs::OdometryConstPtr &state)
{
...
void TrackingGridMap::gazeboCloudCallback(const sensor_msgs::PointCloud2ConstPtr &point)
{
    /***
     * gazebo 雷达数据回调
     */
...
void TrackingGridMap::realsenseCloudCallback(const sensor_msgs::PointCloud2ConstPtr &point){  // 435点膨胀半径小
    /***
...
void TrackingGridMap::lidarCloudCallback(const sensor_msgs::PointCloud2ConstPtr &point){
    /***
     * 实车 雷达数据回调
     */
    static pcl::PointCloud<pcl::PointXYZ>::Ptr local_cloud_3d(new pcl::PointCloud<pcl::PointXYZ>);
...
cv::Mat TrackingGridMap::swellOccMap(cv::Mat occ_map)
{
...
void TrackingGridMap::bevMapToHeight(const cv::Mat bev_map, const cv::Mat occ_map)
{
    /// 高度地图建立！
    std::vector<cv::Mat> channels;
...
void TrackingGridMap::pruneGraph(cv::Mat &topo_map)
{  // TODO 暂时废弃
...
void TrackingGridMap::topoSampleMap(cv::Mat topo_map)
{   // 建立topo采样路径点
    ...
    ROS_WARN("[GridMap] topo_sample_map size: %zu", topo_sample_map.size());
    ROS_WARN("[GridMap] topo_keypoint size: %zu", topo_keypoint.size());
}


void TrackingGridMap::occMaptoObstacle(const cv::Mat occ_map)
{
    pcl::PointXYZ pt;
 ...
 * @param
 */
void TrackingGridMap::localPointCloudToObstacle(const pcl::PointCloud<pcl::PointXYZ> &cloud, const bool &swell_flag, const Eigen::Vector3d current_position)
{
 ...
 * @param   组成障碍物空间点坐标
 */
void TrackingGridMap::setObs(const double coord_x, const double coord_y)
{
 ...
 * @param   组成障碍物空间点坐标
 */
void TrackingGridMap::localSetObs(const double coord_x, const double coord_y, const double coord_z)
...
void TrackingGridMap::setRadiusDash(const double dash_radius)
{
    m_robot_radius_dash = dash_radius;
}

void TrackingGridMap::processSecondHeights()
...
void TrackingGridMap::setHeight(const double coord_x, const double coord_y, const double coord_z, const bool exist_heights)
{
    if (coord_x < gl_xl || coord_y < gl_yl ||
        coord_x >= gl_xu || coord_y >= gl_yu )
        return;
...
double TrackingGridMap::getHeight(const int idx_x, const int idx_y)
{
...
double TrackingGridMap::getSecondHeight(const int idx_x, const int idx_y)
{
...
void TrackingGridMap::resetUsedGrids()
{
    for (int i = 0; i < GLX_SIZE; i++)
        for (int j = 0; j < GLY_SIZE; j++)
        {
...
Eigen::Vector3d TrackingGridMap::gridIndex2coord(const Eigen::Vector3i &index)
{
    Eigen::Vector3d pt;
...
Eigen::Vector3i TrackingGridMap::coord2gridIndex(const Eigen::Vector3d &pt) {
    Eigen::Vector3i idx;
...
void TrackingGridMap::coord2gridIndex(double &pt_x, double &pt_y, double &pt_z, int &x_idx, int &y_idx, int &z_idx)
{
...
inline void TrackingGridMap::resetGrid(GridNodePtr ptr)
{
    ptr->id = 0;
    ptr->cameFrom = NULL;
    ptr->gScore = inf;
...
bool TrackingGridMap::isOccupied(const Eigen::Vector3i &index, bool second_height) const
{
    return isOccupied(index(0), index(1), index(2), second_height);
}

bool TrackingGridMap::isStaticOccupied(const int &idx_x, const int &idx_y, bool second_height) const
{
...
bool TrackingGridMap::isStaticOccupied(const Eigen::Vector3i &index, bool second_height) const
{
    return isStaticOccupied(index(0), index(1), second_height);
}

bool TrackingGridMap::isLocalOccupied(const Eigen::Vector3i &index) const
{
    return isLocalOccupied(index(0), index(1), index(2));
}

bool TrackingGridMap::isFree(const Eigen::Vector3i &index) const
{
    return isFree(index(0), index(1), index(2));
}

bool TrackingGridMap::isOccupied(const int &idx_x, const int &idx_y, const int &idx_z, bool second_height) const
...
bool TrackingGridMap::isLocalOccupied(const int &idx_x, const int &idx_y, const int &idx_z) const
{
    // (Fix 51a) Out-of-bounds = occupied
    if(idx_x < 0 || idx_x >= GLX_SIZE || idx_y < 0 || idx_y >= GLY_SIZE)
        return true;
...
bool TrackingGridMap::isFree(const int &idx_x, const int &idx_y, const int &idx_z) const
{
